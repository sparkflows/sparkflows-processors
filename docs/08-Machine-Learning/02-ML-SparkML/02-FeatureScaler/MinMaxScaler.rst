...
* We will first create a single feature vector using the **VectorAssembler** node that will combine the given list of columns into a single vector column.
* Next we will use the generated vector column as the input column for the **MinMaxScaler** node.
* Keeping the **Min** and **Max** value as default, this node will transforms data by scaling features to the given range. It scales the values to a specific value range without changing the shape of the original distribution.
